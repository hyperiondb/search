...
EXT="hsearch"
PKG="postgresql-${PG_MAJOR}-hsearch"
...
Description: BM25 full-text search for PostgreSQL (Tantivy-backed bm25 index access method)
 ...
 &&& match operator. Built with pgrx.
EOF
...
set -e
cat <<'MSG'
hsearch installed. To enable it:
  1) add it to shared_preload_libraries in postgresql.conf:
       shared_preload_libraries = 'hsearch'      # alongside any others
  2) restart PostgreSQL
  3) in your database:  CREATE EXTENSION hsearch;
See https://github.com/hyperiondb/hsearch for usage.
MSG
EOF
